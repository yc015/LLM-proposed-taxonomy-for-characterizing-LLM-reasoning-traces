# From DeepSeek Math https://github.com/deepseek-ai/DeepSeek-Math/blob/main/evaluation/data_processing/answer_extraction.py#L177


import re
import regex

def extract_gpqa_answer(answer):
    # Try "the correct answer is" format
    answer_lower = answer.lower()
    if "the correct answer is" in answer_lower:
        answer_loc = answer_lower.rfind("the correct answer is") + len("the correct answer is ")
        answer_snippet = answer[answer_loc: answer_loc + 8]
        for i, letter in enumerate("ABCD"):
            if letter in answer_snippet:
                return i
            
    if "answer:" in answer_lower:
        answer_loc = answer_lower.rfind("answer:") + len("answer:")
        answer_snippet = answer[answer_loc: answer_loc + 6]
        for i, letter in enumerate("ABCD"):
            if letter in answer_snippet:
                return i

    # Try boxed format (assuming extract_boxed_answers exists)
    try:
        boxed = strip_string(extract_boxed_answers(answer))
        if boxed and any(c in boxed for c in "ABCD"):
            for i, letter in enumerate("ABCD"):
                if letter in boxed:
                    return i
    except:
    ...
    try:
        if '"answer"' in answer_lower:
            ...
            match = re.search(r'"answer"\s*:\s*"([A-D])"', answer)
            if match:
...
def extract_cruxeval_answer(text):
    """
    Extracts the content between [ANSWER] and [/ANSWER], 
    strips surrounding spaces and newlines, and returns the result.
...
def extract_cruxeval_input_answer(text):
    """
    Extracts the content inside f(...) from between [ANSWER] and [/ANSWER] tags.
    Strips surrounding spaces and newlines, and returns the result.
    Args:
        text (str): The input text containing the answer.
    Returns:
        str: The extracted string inside f(...) or an empty string if not found.
    """
    # Step 1: Extract between [ANSWER] and [/ANSWER]
    answer_pattern = r'\[ANSWER\](.*?)\[/ANSWER\]'
    answer_match = re.search(answer_pattern, text, re.DOTALL)
    if not answer_match:
        return ""
    answer_content = answer_match.group(1).strip()
    # Step 2: Extract inside f(...)
    f_pattern = r'f\((.*?)\)'
    f_match = re.search(f_pattern, answer_content, re.DOTALL)
    if f_match:
...
def extract_aime_answer(answer):
    extracted_answers = extract_boxed_answers(answer)
    if len(extracted_answers) == 0:
        return ""
    else:
        return strip_string(extracted_answers[-1])
