...
show_usage() {
    ...
    echo "Pairs are batched using compared_models_1 and compared_models_2 parameters."
    echo ""
    echo "Arguments:"
    ...
    echo ""
    echo "Note: This script is designed for scripts that accept compared_models_1 and compared_models_2 parameters."
    echo "If there's an odd number of pairs, the last batch will have compared_models_2 set to empty string."
...
# Function to run batched cross-pairs on the specified script
run_batched_cross_pairs() {
    local script_path="$1"
    ...
    echo "========================================="
    echo "Running $script_name on batched cross-model pairs"
    ...
    echo "Total batches: $(( (total_pairs + 1) / 2 ))"
    echo "========================================="
    
    # Process pairs in batches of 2
    for ((i=0; i<total_pairs; i+=2)); do
        batch_count=$((batch_count + 1))
        compared_models_1="${all_pairs[i]}"
        
        # Check if there's a second pair in this batch
        if ((i+1 < total_pairs)); then
            compared_models_2="${all_pairs[i+1]}"
        else
            compared_models_2=""  # Empty string for odd number of pairs
        fi
        
        echo ""
        echo "========================================="
        echo "Running batch $batch_count:"
        echo "  compared_models_1: $compared_models_1"
        echo "  compared_models_2: $compared_models_2"
        echo "Script: $script_path"
        echo "========================================="
        ...
        # Run the script with the current batch
        sbatch "$script_path" "$compared_models_1" "$compared_models_2"
...
list_models() {
    ...
    echo "Total batches: $(( (${#models_list1[@]} * ${#models_list2[@]} + 1) / 2 ))"
}
...
dry_run() {
    ...
    for ((i=0; i<total_pairs; i+=2)); do
        ...
        echo "  compared_models_1: $compared_models_1"
        echo "  compared_models_2: $compared_models_2"
        echo "  Command: sbatch $script_path \"$compared_models_1\" \"$compared_models_2\" "
        echo ""
    done
...
if [[ ! -f "$script_path" ]]; then
    echo "Error: Script not found: $script_path"
    echo "Please provide a valid path to a vLLM coder script that supports batched comparisons"
    exit 1
fi
...
# Run all batched cross-pairs
run_batched_cross_pairs "$script_path"
