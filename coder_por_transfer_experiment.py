...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train POR coder on dataset A and evaluate on dataset B (and vice versa)."
    )
    ...
    parser.add_argument("--refit_transfer_classifier", type=str, default="no", choices=["yes", "no"])
...
def create_coder(args, dataset_path, model_options, codebook=None):
    ...
    if "cruxeval" in dataset_path.lower() or "execution" in dataset_path.lower():
        min_num_train_samples = 300
    elif "gpqa" in dataset_path.lower():
        min_num_train_samples = 100
    elif "aime" in dataset_path.lower():
...
def prepare_dataset_sections(
    ...
    warm_count = min(int(max(1, warmup_num)), len(dataset))
...
def run_training_cycle(
    ...
    folder = os.path.join(
        output_dir,
        f"{os.path.basename(dataset_path.rstrip('/'))}-PoR-transfer",
    )
    os.makedirs(folder, exist_ok=True)
    intermediate = os.path.join(folder, "intermediate_ckpt")
    os.makedirs(intermediate, exist_ok=True)

    warm_attr = os.path.join(intermediate, f"{tag}_warmup_{args.job_id}.pkl")
    train_attr = os.path.join(intermediate, f"{tag}_train_{args.job_id}.pkl")
    ...
    in_domain_acc = evaluate_dataset(coder, eval_dataset, args, f"{tag} eval split")
    coder_path = os.path.join(
        folder,
        f"coder-por-{tag}-{args.job_id}.pkl",
    )
...
def main():
    ...
    summary_path = os.path.join(
        base_output,
        f"por_transfer_summary_{os.path.basename(args.dataset_a_path.rstrip('/'))}_"
        f"{os.path.basename(args.dataset_b_path.rstrip('/'))}_{args.job_id}.txt",
    )
    with open(summary_path, "w") as f:
        f.write("POR Transfer Experiment Results\n")
