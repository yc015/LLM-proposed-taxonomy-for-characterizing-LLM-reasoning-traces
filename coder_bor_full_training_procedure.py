...
parser = argparse.ArgumentParser(description="Generate responses using a specified model with CoderBOR.")
...
parser.add_argument('--batch_size', type=int, default=8, help='Batch size for evaluation and accumulation')
parser.add_argument('--evaluation_method', type=str, default="generative", help='Method used in evaluation: ["generative", "hard_vote", "soft_vote", "naive_bayes", "multinomial_nb", "cosine_similarity", "euclidean_similarity", "knn", "logistic_regression"]')
...
parser.add_argument('--vllm', action='store_true', help='Use vLLM for faster inference')
...
parser.add_argument('--normalize', action='store_true', help='Normalize vector before training classifier')
parser.add_argument('--normalization_method', type=str, default="comparative", help='Options: "comparative", "l1", "l2"')
...
parser.add_argument('--multi_gpus', type=int, default=1, help='Number of GPUs to use for vLLM tensor parallelism')

parser.add_argument('--reject_inconsistent_codes', action='store_true', help='Reject inconsistent code before evaluation.')
parser.add_argument('--averaged_eval', action='store_true', help='Use averaged evaluation')
parser.add_argument('--num_reruns', type=int, default=5, help='Number of reruns for averaged evaluation')
parser.add_argument('--existing_codebook_path', type=str, default=None, help='Path to the existing codebook')
...
if args.output_path is None:
    # Create a folder name based on the dataset
    dataset_folder = args.dataset_path.split('/')[-1].split('.')[0]  # Assuming the dataset file has an extension
    output_folder = f"/n/home04/yidachen/reasoning_characteristics/coder_ckpt/{dataset_folder}-BoR{fixed_codebook_baseline}"
    intermediate_output_folder = f"/n/home04/yidachen/reasoning_characteristics/coder_ckpt/{dataset_folder}-BoR{fixed_codebook_baseline}/intermediate_ckpt"
    ...
    coder_type = "bor"
    args.output_path = (
        f"{output_folder}/coder-{coder_type}-{args.normalization_method}-{args.coder_model_id.split('/')[-1]}"
        f"-compare-{'_'.join(model_options)}-on-{dataset_folder}-{args.job_id}.pkl"
    )
    intermediate_output_path = (
        f"{intermediate_output_folder}/coder-{coder_type}-{args.normalization_method}-{args.coder_model_id.split('/')[-1]}"
        f"-compare-{'_'.join(model_options)}-on-{dataset_folder}-{args.job_id}.pkl"
    )
...
# Use CoderBOR class
print("Using CoderBOR (Bag of Reasoning) version of the coder")
# CoderClass = CoderBOR
...
if "aime" in args.dataset_path.lower():
    print("Use 108k context length for AIME dataset due long averaged reasoning lengths.")
...
if args.existing_codebook_path is not None and os.path.exists(args.existing_codebook_path):
    print("Use existing codebook")
    with open(args.existing_codebook_path, "rb") as infile:
        coder = pickle.load(infile)
    initial_codebook = coder.codebook
    print("Loaded codebook from", args.existing_codebook_path)
    print("Codebook size", len(initial_codebook))
    print("Codebook", initial_codebook)
else:
...
# coder.codebook = INITIAL_CODEBOOK
...
# ### Training with accumulate observation training
print(f"Using accumulate observation training with accumulation size: {args.accumulation_size}")
attr_path = intermediate_output_path
...
# Use sequential evaluation for CoderBOR
print("Using batched evaluation for CoderBOR")

if args.reject_inconsistent_codes:
    filtered_results, filtered_codebook = coder.reject_inconsistent_codes(random.sample(train_dataset, min(50, len(train_dataset))), num_reruns=args.num_reruns, modify_training_data=True)

if args.averaged_eval:
    coder.eval_with_averaged_vectors(eval_dataset, num_reruns=args.num_reruns, variance_calculation=True)
else:
