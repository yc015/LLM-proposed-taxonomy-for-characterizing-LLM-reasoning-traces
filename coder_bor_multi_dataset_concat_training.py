...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train BOR coder on concatenated datasets with stratified splits."
    )
...
def create_coder(args, dataset_paths, model_options, codebook=None):
    ...
    if any("aime" in path.lower() for path in dataset_paths):
...
def main():
    print("Start the multi-dataset training")
    args = parse_args()
    print("Parsed Input")
    ...
    folder = os.path.join(output_base, f"{dataset_tag}-BoR-multidataset")
    ...
    coder_path = os.path.join(
        folder,
        f"coder-bor-multidataset-{dataset_tag}-{args.job_id}.pkl",
    )
    coder.save_coder(coder_path)

    summary_path = os.path.join(folder, f"summary-{args.job_id}.txt")
    with open(summary_path, "w") as f:
        f.write("BOR Multi-dataset Training Summary\n")
